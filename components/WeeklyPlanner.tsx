  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4 animate-in fade-in slide-in-from-bottom-6 duration-500">
      <div className="flex items-center justify-between mb-10">
        ...
        <h2 className="text-2xl font-serif font-black text-slate-900 dark:text-white">AI Haftalık Planlayıcı</h2>
      </div>
      ...
      {isConfiguring || (!plan && !loading) ? (
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] shadow-xl border border-slate-100 dark:border-slate-800">
          <div className="text-center mb-8">
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-white">Çalışma Saatlerini Belirle</h3>
            <p className="text-slate-500 text-sm mt-2">Hangi günlerde, hangi saatlerde müsaitsin? Yapay zeka programı bu saatlere göre yapacak.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            {availability.map((a) => (
              <div key={a.day} className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl">
                <h4 className="font-bold text-indigo-600 mb-3 text-sm">{a.day}</h4>
                    ...
                    <button
                      ...
                      className={`py-2 rounded-lg text-[10px] font-black transition-all ${
                        a.slots.includes(time)
                          ...
                          : 'bg-white dark:bg-slate-800 text-slate-400 dark:text-slate-500 hover:bg-indigo-50 dark:hover:bg-slate-700'
                      }`}
                    >
                      {time}
                    </button>
          ...
          <div className="flex justify-center gap-4">
            ...
            <button 
              ...
              className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl hover:scale-105 transition-all"
            >
              {plan ? "Programı Güncelle" : "Haftalık Programımı Oluştur"}
            </button>
          </div>
        </div>
      ) : (
        <div className="space-y-8">
          <div className="bg-indigo-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
            <div className="flex justify-between items-start relative z-10">
               <div>
                <h3 className="text-xl font-serif font-bold mb-2 flex items-center gap-3">
                  <span>🎯</span> Yapay Zeka Tavsiyesi
                </h3>
                <p className="text-indigo-100 text-sm italic opacity-90 leading-relaxed max-w-2xl">
                  "{plan?.advice}"
                </p>
               </div>
               <button 
            ...
            <div className="absolute right-[-20px] bottom-[-20px] opacity-10 text-[150px] rotate-12">💡</div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {DAYS.map(day => {
              ...
              return (
                <div key={day} className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-100 dark:border-slate-800 p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between mb-4 border-b border-slate-50 dark:border-slate-800 pb-2">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{daySessions?.length || 0} Oturum</span>
                  </div>
                  
                  <div className="space-y-3">
                    {!daySessions || daySessions.length === 0 ? (
                      <p className="text-[10px] text-slate-400 italic py-4">Bu gün için seçtiğin saatlere ders atanmadı.</p>
                    ) : (
                      daySessions.map((session, i) => (
                        <div key={i} className="group p-4 bg-slate-50 dark:bg-slate-950 rounded-2xl border border-transparent hover:border-indigo-100 transition-all">
                          ...
                          <h5 className="text-sm font-bold text-slate-800 dark:text-white mb-1">{session.courseName}</h5>
                          ...
                          <button 
                            ...
                            className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-2 rounded-xl text-[10px] font-black text-slate-600 dark:text-slate-300 hover:bg-slate-900 hover:text-white transition-all shadow-sm"
                          >
                            📅 Takvime Ekle
                          </button>
                        </div>
          ...
          <div className="flex justify-center pt-8">
            ...
            >
              🔄 Saatlerimi Değiştir ve Yeniden Hazırla
            </button>
          </div>
        </div>
      )}
